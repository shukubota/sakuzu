...
for i in range(1,9):
	...
	if which==1:
		...
		plotdata=np.loadtxt("/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu21/plotdata/c"+"%02d"%i+".dat",delimiter=",")

	elif which==2:
		plotdata=np.loadtxt("/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu21/plotdata/phi"+"%02d"%i+".dat",delimiter=",")
	else:
		print "error"
	print np.shape(plotdata)
   
   
	plotdata1=np.load("/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu31/winddata_bimodal1211/uphase"+str(i)+".npy")
	plotdata2=np.load("/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu31/winddata_bimodal1211/vphase"+str(i)+".npy")
	...
	m.drawparallels(np.arange(lat3,lat4,1.),labels=[True,False,False,True])    #緯線
	m.drawmeridians(np.arange(lon3-2,lon4,2.),labels=[False,True,False,True])	#経線
	...
	m.colorbar()
	if which==1.:
		output="figure/c_sm/c"+"%02d"%i+".png"
		plt.title("Amplitude of Diurnal Precipitation [Phase="+"%02d"%i+"] [mm/h]")
	 
	if which==2:
		output="figure/phi_sm/phi"+"%02d"%i+".png"
	...
	if which==1:
		mabiki=2
		q=m.quiver(x[::mabiki,::mabiki],y[::mabiki,::mabiki],plotu[::mabiki,::mabiki],plotv[::mabiki,::mabiki],scale=24,color="black")
		qk = plt.quiverkey(q, 1.1, -0.028, 1, '1 m/s', labelpos='W',color="black")
		print "test\n"
	else:
		mabiki=2
		#q=m.quiver(x[::mabiki,::mabiki],y[::mabiki,::mabiki],plotu[::mabiki,::mabiki],plotv[::mabiki,::mabiki],scale=24,color="black")

		#qk = plt.quiverkey(q, 0.5, -0.062, 5, '850hPa Wind Speed 5 m/s', labelpos='W',color="black")
		print "test\n"
...
os.system("convert -delay 80 -loop 0 figure/c_sm/c*.png figure/c_sm.gif")
os.system("convert -delay 80 -loop 0 figure/phi_sm/phi*.png figure/phi_sm.gif")
