...
for i in range(1,9):
	...
	if which==1:
		...
		plotdata=np.loadtxt("/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu21/plotdata/anmc"+"%02d"%i+".dat",delimiter=",")
	...
	if which==1:
		m.contourf(x, y, plotcp,np.arange(-0.5,0.8,0.1),extend="both")
		#カラーバー逆にしたかったらYlGn_rとかく
		print "test\n"
	else:
	...
	if which==2:
		output="figure/phi/phi"+"%02d"%i+".png"
	...
	if which==1:
		mabiki=3
		q=m.quiver(x[::mabiki,::mabiki],y[::mabiki,::mabiki],plotu[::mabiki,::mabiki],plotv[::mabiki,::mabiki],scale=130,color="black")
		qk = plt.quiverkey(q, 1.08, -0.06, 5, '5 m/s', labelpos='W',color="black")
		print "test\n"
	else:
