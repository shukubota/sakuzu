...
#			comand="sshpass -p 'mitsuyacider' scp kubota@10.226.168.181:/disc10/yamamoto/GRIBdata/kubota/"+str(year)+"/"+str(int(dhead[date]))+"prs-uv.grib data/"
			comand="sshpass -p 'mitsuyacider' scp -o StrictHostKeyChecking=no -P 20022 kubota@localhost:/disc10/yamamoto/GRIBdata/kubota3/"+str(year)+"/"+str(int(dhead[date]))+"prs-q.grib data/"
		...
		if len(list)==0:
			...
			continue
		print list[0]
		if (list[0]=="/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu31/data/20051120prs-q.grib"):
			os.system("rm data/*")
			continue
	...
	np.save("hum/uphase"+str(phase)+".npy",u)
	#np.save("hum/vphase"+str(phase)+".npy",v)
