...
for phase in range(1,9):
			...
			if len(array)!=0:
				...
				for k in range(0,len(datacell)):
					if datacell[k]>0.:
						...
						elif (flag[k]>=200) and (flag[k]<300):
							plotdata2[hourint[k]][mint[k]][nint[k]]+=datacell[k] 
	...
	for p in range(0,24):
		hourname="%02d"%p
		filename="plotdata/"+phasename+"/count"+hourname+".dat"
	...
	for p in range(0,24):
		hourname="%02d"%p
		filename="plotdata/"+phasename+"/"+hourname+".dat"
		fp=open(filename,"w")
