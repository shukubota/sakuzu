...
for phase in range(1,9):
	...
	#filename2="plotdata/wh04/"+dire+"/f"+"%d"%phase+".dat"
	...
	fig.text(0.4,0.92,"Phase%02d"%phase,fontsize=15)
	...
	#cax=fig.add_axes([0.4,0.1,0.04,0.8])
	plt.colorbar(cf,cax)
	...
	#plt.subplot(313)
	#X,Y=np.meshgrid(lonp,[1])
	#plt.contourf(X,Y,plotdata2,np.arange(0.5,0.8,0.1),extend="max")
	#plt.colorbar()
	#plt.plot(lonp,plotdata2)

	#axs1.tick_params(labelbottom='off')
	#cax=fig.add_axes([0.9,0.05,0.6,0,015])
	#plt.colorbar(t1,cax,orientation="vertical")
	plt.savefig("figure/wh04_"+dire+"/hov_wh04_p"+"%d"%phase+".png",bbox_inches="tight")
	plt.clf()	
