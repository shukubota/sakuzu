...
for i in range(1,2):
	...
	if which==1:
		...
		plotdata=np.load("/home/kubotashu/kubota/labo/sakuzu/sakuzu/sakuzu39/plotdata/ffor8.npy")
	...
	if which==1:
		#plotcp[plotcp>3.1]=3.2
		#a=m.contourf(x, y, plotcp,np.arange(-0.3,0.35,0.05),cmap=cm.seismic,extend="both")
		
		#m.contour(x,y,plotcp,levels=[0.,3.1,10.])
		m.contourf(x,y,plotcp,np.arange(3.47,347000000,1000000),extend="max",cmap=cm.brg_r)
		np.save("plotdata/fsakuzu%02d.npy"%i,plotcp)
		print "test\n"
		#b=plt.colorbar(a,orientation="horizontal",shrink=0.8)
		#b.set_label("mm/h")
		#m.colorbar()
	else:
		a=m.contourf(x, y, plotcp,levels=[0.,3.1,10.])
